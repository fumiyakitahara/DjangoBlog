...
class ContactForm(forms.Form):
  ...
  def __init__(self, *args, **kwargs):
    '''ContactFormのコンストラクター
       ...
       フィールドの初期化を行う
       CharFieldならTextInputなどフィールドごとにデフォルトのwigetが存在する
  ...
  def send_email(self):
    ...
    cleaned_data フォームがバリデーションをパスした場合、すべてのフィールドのデータを読み込む
    djangoにはメール送信を行うクラスが用意されている(subject,body,from_email,to,bcc,cc,attchments,headers,reply_to)
